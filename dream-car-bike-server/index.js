...
async function run() {
    try {
        ...
        const bikeCollection = client.db('dream_car_bikeDB').collection('bike')
...
app.get('/', (req, res) => {
    res.send('dream car bike server is running')
})

app.listen(port, () => {
    console.log(`DREAM CAR BIKE SERVER IS RUNNING ON PORT : ${port}`);
})